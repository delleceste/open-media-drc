...
Set `os = FreeBSD` (or another value returned by Python's
`platform.system()`) to expose a command only on that operating system. Several
may be listed, comma-separated (`os = Linux, FreeBSD`). Commands for other
operating systems are omitted from both the page and the `/run/<id>` API.
