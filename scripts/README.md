...
| `headroom_calc.py` | Computes the minimum `attenuation:` value for each brutefir `.conf` from the filters' worst-case gain — steady-state FFT peak, impulse step response, AND real convolution against the fabricated stress master below, since magnitude-only tests miss excess-phase overshoot on real program material (+ safety margin, default 1 dB) — so playback never clips while dynamics are maximised. Run it after every filter (re)generation. | Linux + FreeBSD (python3) |
| `gen-stress-master-wav.py` | Generates the fabricated, clean-room "brickwall master" WAV (`stress-master-48000-mono.wav`, committed) that `headroom_calc.py` convolves through each filter — a real, full-scale, low-crest program signal is the only thing that bounds excess-phase overshoot; see its docstring for the 2026-09-15 incident that made this necessary. | Linux + FreeBSD (python3) |
